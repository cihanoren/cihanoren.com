...
<div class="lux -mt-20">
    ...
    <section class="relative min-h-[92vh] flex flex-col">
        ...
        <div class="relative flex-1 flex flex-col justify-center max-w-7xl mx-auto px-6 w-full pt-28 pb-10" style="z-index:2;">
            ...
            $heroBadge = Setting::get("hero_badge_{$locale}", Setting::get('hero_badge', __('messages.home_badge')));
            $heroTitle = Setting::get("hero_title_{$locale}", Setting::get('hero_title', 'Flutter Developer & Mobile Architect'));
            $heroSubtitle = Setting::get("hero_subtitle_{$locale}", Setting::get('hero_subtitle', __('messages.home_hero_sub')));
        ...
        <div class="relative max-w-7xl mx-auto px-6 w-full pb-14" style="z-index:2;">
            ...
            $stats = json_decode(Setting::get('home_stats', json_encode($defaultStats)), true) ?: $defaultStats;
            $statLabelKey = 'label_' . app()->getLocale();
            @endphp
            <div class="grid gap-6 md:gap-12 pt-8 border-t border-white/10"
                style="grid-template-columns: repeat({{ count($stats) }}, minmax(0, 1fr));">
                @foreach($stats as $stat)
                @php
                $value = $stat['value'] ?? '';
                $label = $stat[$statLabelKey] ?? ($stat['label_tr'] ?? '');
                @endphp
                <div class="boot b6">
                    <p class="display text-white font-semibold {{ strlen($value) > 4 ? 'text-xl md:text-3xl' : 'text-3xl md:text-5xl' }}">{{ $value }}</p>
                    ...
                    <p class="mono text-[11px] md:text-[12px] tracking-wide uppercase text-zinc-500">{{ $label }}</p>
                </div>
                @endforeach
            </div>
        </div>
                    ...
                    <div id="cta-messages" class="px-5 py-4 space-y-3 overflow-y-auto" style="min-height: 380px; max-height: 520px;">
                        <div class="flex gap-2.5">
                            <span class="w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5"
                                style="background: linear-gradient(135deg,#22d3ee,#818cf8,#c084fc);">
                                <svg class="w-3 h-3 text-black/80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    ...
    (function() {
        ...
        function ctaExpand(el, dur) {
            ...
            return ctaWait(ctaReduce ? 0 : dur).then(function() {
        ...
        function fxDraw() {
            ...
            fxCtx.fillStyle = g;
            fxCtx.beginPath();
            fxCtx.arc(fxHead.x, fxHead.y, 7, 0, 6.2832);
            fxCtx.fill();
            fxCtx.beginPath();
            fxCtx.arc(fxHead.x, fxHead.y, 1.9, 0, 6.2832);
            fxCtx.fillStyle = 'rgba(255,255,255,' + fxAlpha + ')';
            fxCtx.fill();
        }

        function fxShow(pt) {
            fxResize();
            ctaFx.style.display = 'block';
            fxAlpha = 1;
            fxTrail = [];
            fxHead.x = pt.x;
            fxHead.y = pt.y;
        ...
        function ctaBtnPoint() {
            ...
            return {
                x: sr.left + ctaBtnOffset.x,
                y: sr.top + ctaBtnOffset.y
            };
        }
        window.addEventListener('resize', function() {
            if (ctaFx && ctaFx.style.display !== 'none') fxResize();
        });
        ...
        window.ctaOpenChat = async function() {
            ...
            if (!ctaFx || ctaReduce || ctaIOS) {
                ...
                ctaAnimating = false;
                const i = document.getElementById('cta-input');
                if (i) i.focus();
                return;
            }
            ...
            await bloom;
            ctaAnimating = false;
            const inp = document.getElementById('cta-input');
            if (inp) inp.focus();
        };
