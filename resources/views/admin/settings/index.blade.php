...
<form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data">
    ...
    <div class="max-w-2xl space-y-6">
        ...
        <div class="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 space-y-5">
            ...
            <p class="text-xs text-gray-500 -mt-2">Anasayfa hero alanı. İki dili de doldurun.</p>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    <p class="mono text-[11px] uppercase tracking-wide text-cyan-400">Türkçe</p>
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Rozet Metni</label>
                        <input type="text" name="hero_badge_tr" value="{{ old('hero_badge_tr', $settings['hero_badge_tr']) }}"
                            ...
                            placeholder="Freelance çalışmaya açık">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">
                            Hero Başlık <span class="normal-case font-normal text-gray-600">(& işaretinden sonrası vurgulanır)</span>
                        </label>
                        <input type="text" name="hero_title_tr" value="{{ old('hero_title_tr', $settings['hero_title_tr']) }}"
                            class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60 transition-all"
                            placeholder="Flutter Geliştirici & Mobil Mimar">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Alt Metin</label>
                        <textarea name="hero_subtitle_tr" rows="3"
                            class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60 transition-all resize-none"
                            placeholder="Mimari, performans ve kullanıcı deneyimine odaklanan...">{{ old('hero_subtitle_tr', $settings['hero_subtitle_tr']) }}</textarea>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">
                            Hero Title <span class="normal-case font-normal text-gray-600">(part after & is highlighted)</span>
                        </label>
                        <input type="text" name="hero_title_en" value="{{ old('hero_title_en', $settings['hero_title_en']) }}"
                            class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60 transition-all"
                    ...
                    <div>
                        ...
                        <textarea name="hero_subtitle_en" rows="3"
                            class="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60 transition-all resize-none"
                            placeholder="I build clean, scalable mobile applications...">{{ old('hero_subtitle_en', $settings['hero_subtitle_en']) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 space-y-5">
            <div class="flex items-center justify-between">
                <h2 class="text-sm font-bold text-white">Home Page Stats</h2>
                <button type="button" id="add-stat" class="text-xs font-medium text-indigo-400 hover:text-indigo-300 inline-flex items-center gap-1">
            ...
            <p class="text-xs text-gray-500 -mt-3">Anasayfa hero altındaki "3+ Yıl Deneyim" gibi kutular. Değer aynı kalır, etiket iki dilde ayrı girilir.</p>

            <div id="stat-rows" class="space-y-4">
                @foreach($homeStats as $i => $stat)
                <div class="stat-row grid grid-cols-1 md:grid-cols-[120px_1fr_1fr_auto] gap-3 items-start p-4 rounded-xl border border-white/[0.06] bg-white/[0.015]">
                    <input type="text" name="stats[{{ $i }}][value]" value="{{ $stat['value'] }}" placeholder="3+"
                    ...
                    <input type="text" name="stats[{{ $i }}][label_tr]" value="{{ $stat['label_tr'] }}" placeholder="YIL DENEYİM (TR)"
                        class="w-full px-3 py-2.5 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60">
                    <input type="text" name="stats[{{ $i }}][label_en]" value="{{ $stat['label_en'] }}" placeholder="YEARS EXPERIENCE (EN)"
                        class="w-full px-3 py-2.5 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60">
                    <button type="button" class="remove-stat h-10 w-10 flex items-center justify-center rounded-lg text-red-400 hover:bg-red-500/10 transition-colors">
...
<script>
    ...
    (function() {
        ...
        addBtn.addEventListener('click', () => {
            const i = rows.children.length;
            const row = document.createElement('div');
            row.className = 'stat-row grid grid-cols-1 md:grid-cols-[120px_1fr_1fr_auto] gap-3 items-start p-4 rounded-xl border border-white/[0.06] bg-white/[0.015]';
            row.innerHTML = `
            <input type="text" name="stats[${i}][value]" placeholder="3+" class="w-full px-3 py-2.5 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60">
            <input type="text" name="stats[${i}][label_tr]" placeholder="YIL DENEYİM (TR)" class="w-full px-3 py-2.5 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60">
            <input type="text" name="stats[${i}][label_en]" placeholder="YEARS EXPERIENCE (EN)" class="w-full px-3 py-2.5 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-indigo-500/60">
            <button type="button" class="remove-stat h-10 w-10 flex items-center justify-center rounded-lg text-red-400 hover:bg-red-500/10 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>`;
            rows.appendChild(row);
